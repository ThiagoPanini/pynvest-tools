...
resource "aws_glue_catalog_table" "tbsor_fundamentus_indicadores_brutos_acoes" {
  name          = local.tables_names_map["fundamentus"]["sor_acoes"]
  ...
  storage_descriptor {
    location      = "s3://${var.bucket_names_map["sor"]}/${local.tables_names_map["fundamentus"]["sor_acoes"]}"
    input_format  = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat"
    output_format = "org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat"

    ser_de_info {
      name                  = "stream-${local.tables_names_map["fundamentus"]["sor_acoes"]}"
    ...
    dynamic "columns" {
      for_each = jsondecode(file("${path.root}/infra/modules/catalog/schemas/${local.tables_names_map["fundamentus"]["sor_acoes"]}.json"))["columns"]
      content {
